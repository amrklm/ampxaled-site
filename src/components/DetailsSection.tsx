...
import { supabase } from "../lib/supabaseClient.ts"; // adjust path if needed
...
const DetailsSection = () => {
  ...
  useEffect(() => {
    const loadImages = async () => {
      const { data, error } = await supabase.storage.from("ampxaled").list("img", {
        limit: 100,
        offset: 0,
      ...
      if (!data || data.length === 0) {
        console.warn("⚠️ No images found in img/");
        return;
      }
      ...
      const urls = data
        ...
        .map((file) =>
          `https://nozammxkdbmqmxcpjmte.supabase.co/storage/v1/object/public/ampxaled/img/${file.name}`
        );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      toast.error("Please fill in all required fields"); // same error message as before
      return;
    }

    toast.success("Request submitted successfully!"); // same success message
  ...
  return <section id="details" className="w-full bg-white py-0">
    <div className="container px-4 sm:px-6 lg:px-8 mx-auto">
      {/*old two card grid replce with line:42 // grid grid-cols-1 gap-6 md:gap-8 lg:grid-cols-2 */}
      <div className="max-w-3xl mx-auto text-center mb-8 sm:mb-12">
        {/* Left Card - The Details */}
        {/* Here card left false hidden */}
        {false && (
          <div className="w-full max-w-2xl rounded-2xl sm:rounded-3xl overflow-hidden shadow-elegant">
